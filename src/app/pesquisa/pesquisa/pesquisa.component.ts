...
export class PesquisaComponent implements OnInit {
  ...
  atualizar(i: number) {
    let tl: Tl = new Tl();
    tl.id = this.listaDeTlsSemOrganizar[i].id;
    tl.data = this.formArray.controls[i].get('data')?.value;
    tl.tanque = this.formArray.controls[i].get('tanque')?.value;
    tl.gordura = this.formArray.controls[i].get('gordura')?.value;
    tl.snf = this.formArray.controls[i].get('snf')?.value;
    tl.crioscopia = this.formArray.controls[i].get('crioscopia')?.value;
    tl.densidade = this.formArray.controls[i].get('densidade')?.value;
    tl.acidez = this.formArray.controls[i].get('acidez')?.value;
    tl.notaFiscal = this.formArray.controls[i].get('notaFiscal')?.value;
    tl.transportadora = this.formArray.controls[i].get('transportadora')?.value;
    tl.codigoCarreta = this.formArray.controls[i].get('codigoCarreta')?.value;
    tl.kilosRecebidos = this.formArray.controls[i].get('kilosRecebidos')?.value;
    tl.materiaPrima = this.formArray.controls[i].get('materiaPrima')?.value;
    tl.codigoMateriaPrima = this.formArray.controls[i].get('codigoMateriaPrima')?.value;
    tl.temperatura = this.formArray.controls[i].get('temperatura')?.value;
    this.pesquisaService.atualiazarTl(tl).subscribe((tl:Tl) => {
      this.referenciaModalSuccess=this.modalService.show(this.success,{class:'modal-dialog-centered'})
      setTimeout(()=>{
       this.referenciaModalSuccess.hide()
      },500)
      this.formArray.controls[i].disable();
    });
